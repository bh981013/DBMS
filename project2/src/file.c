...
pagenum_t file_alloc_page(){
	...
	printf("이제 헤더가 가르키는 free pnum:%ld\n", hpage->header_page.free_pnum);
	return free_num;
	free(hpage);	
}
...
void file_free_page(pagenum_t pagenum){
	...
	file_write_page(header_page_num, &hpage); 
	file_write_page(pagenum, &fpage);			//freepage 와 header page를 write함
